...
import { Share2, Zap, QrCode, Trophy, User, ShieldCheck, Flame } from 'lucide-react';
...
export default function Profile({ onNavigate, userRole }: ProfileProps) {
  ...
  const [streak, setStreak] = useState(0); // Новое состояние для Стрика

  useEffect(() => {
    async function fetchPoints() {
      if (tgUser?.id) {
        // Подтягиваем и points, и streak_count
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-32">
      ...
      <main className="px-6 py-4 space-y-8 overflow-x-hidden">
        ...
        {/* ШКАЛА УРОВНЯ С ДОБАВЛЕНИЕМ СТРИКА */}
        ...
        {/* ... (Остальной код Earn Points и Admin Panel остается без изменений) */}
